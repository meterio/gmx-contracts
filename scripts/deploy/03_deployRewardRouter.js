const { deployContract, contractAt, sendTxn, nativeToken, readTmpAddresses } = require("../shared/helpers")
...
async function main() {
  ...
  const glpManager = await contractAt("GlpManager", GlpManager.address)
  const glp = await contractAt("GLP", GLP.address)

  const gmx = await contractAt("GMX", GMX.address);
  const esGmx = await contractAt("EsGMX", EsGMX.address);
  const bnGmx = await contractAt("MintableBaseToken", bnGMX.address);
  ...
  const stakedGmxTracker = await deployContract("RewardTracker", ["Staked GMX", "sGMX"],"sGMX")
  const stakedGmxDistributor = await deployContract("RewardDistributor", [esGmx.address, stakedGmxTracker.address],"sGMX_RewardDistributor")
  ...
  const bonusGmxTracker = await deployContract("RewardTracker", ["Staked + Bonus GMX", "sbGMX"],"sbGMX")
  const bonusGmxDistributor = await deployContract("BonusDistributor", [bnGmx.address, bonusGmxTracker.address],"sbGMX_BonusDistributor")
  ...
  const feeGmxTracker = await deployContract("RewardTracker", ["Staked + Bonus + Fee GMX", "sbfGMX"],"sbfGMX")
  const feeGmxDistributor = await deployContract("RewardDistributor", [nativeToken.address, feeGmxTracker.address],"sbfGMX_RewardDistributor")
  ...
  const feeGlpTracker = await deployContract("RewardTracker", ["Fee GLP", "fGLP"],"fGLP")
  const feeGlpDistributor = await deployContract("RewardDistributor", [nativeToken.address, feeGlpTracker.address],"fGLP_RewardDistributor")
  ...
  const stakedGlpTracker = await deployContract("RewardTracker", ["Fee + Staked GLP", "fsGLP"],"fsGLP")
  const stakedGlpDistributor = await deployContract("RewardDistributor", [esGmx.address, stakedGlpTracker.address],"fsGLP_RewardDistributor")
  ...
  const rewardRouter = await deployContract("RewardRouterV2", [],"RewardRouter")
